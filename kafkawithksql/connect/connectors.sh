...
echo "\ncreate topics"
docker exec connect bash -c "kafka-topics --create --if-not-exists --zookeeper zookeeper:2181 --topic local-inventory-cdc-dsv-v1 --replication-factor 1 --partitions 1"
docker exec connect bash -c "kafka-topics --create --if-not-exists --zookeeper zookeeper:2181 --topic local-inventory-cdc-preorder-v0 --replication-factor 1 --partitions 1"
docker exec connect bash -c "kafka-topics --create --if-not-exists --zookeeper zookeeper:2181 --topic local-product-cdc-preorder-event-v1 --replication-factor 1 --partitions 1"
